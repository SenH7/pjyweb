      ...
      <section className="py-16 bg-gray-50">
        <div className="container mx-auto px-4">
          ...
          <div className="prose max-w-none">
            <p className="mb-4">
              {locale === 'en'
                ? `The ${product.title.en} is designed for ${product.specifications?.technology === 'PCAP' ? 'modern' : 'industrial'} applications requiring ${product.specifications?.technology === 'PCAP' ? 'multi-touch capability and excellent visual clarity' : 'durability and reliable operation in harsh environments'}. With its ${product.specifications?.resolution || 'high'} resolution and responsive interface, it provides an exceptional user experience.`
                : `${product.title.zh || product.title.en}是为需要${product.specifications?.technology === 'PCAP' ? '多点触控功能和出色视觉清晰度的现代' : '在恶劣环境中耐用性和可靠操作的工业'}应用而设计的。凭借其${product.specifications?.resolution || '高'}分辨率和响应灵敏的界面，它提供了卓越的用户体验。`}
            </p>
            <p className="mb-4">
              {locale === 'en'
                ? `The touchscreen features a ${product.specifications?.dimensions || 'compact'} display area with ${product.specifications?.technology || 'advanced'} technology, making it ideal for ${product.specifications?.technology === 'PCAP' ? 'consumer electronics, retail kiosks, and interactive displays' : 'manufacturing environments, outdoor installations, and specialized equipment'}. Its ${product.specifications?.interface || 'standard'} interface ensures easy integration with various systems.`
                : `触摸屏具有${product.specifications?.dimensions || '紧凑'}的显示区域，采用${product.specifications?.technology || '先进'}技术，使其非常适合${product.specifications?.technology === 'PCAP' ? '消费电子产品、零售终端和交互式显示器' : '制造环境、户外安装和专业设备'}。其${product.specifications?.interface || '标准'}接口确保与各种系统轻松集成。`}
            </p>
            <p>
              {locale === 'en'
